...
{
    uint32_t dest; // destination of this edge in the graph, MAX_INT if this is a 目的：哨兵的作用，可用于返回
                   // sentinel
    uint32_t
        value; // edge value of zero means it a null since we don't store 0 edges，用于存储边的权值，或者trussness值
} edge_t;
...
int main()
{
    ...
    PCSR pcsr = PCSR(1696415);
    ifstream read_file;
    read_file.open("./data/as-skitter.txt");
    int node1, node2;
    cout << "deal................." << endl;
    ...
    for(int i = 0; i < 11095298; i++) {
        ...
        if(i %500000 == 0) {
            cout << i << " edge cost time: " << (double) (clock() - startTime) / CLOCKS_PER_SEC << "s" << endl;
        }
        pcsr.add_edge(node1, node2, 1);
    }
    read_file.close();
    cout << bsf_word(34) << endl;
